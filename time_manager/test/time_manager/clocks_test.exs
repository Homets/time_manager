defmodule TimeManager.ClocksTest do
  ...
  describe "clocks" do
    ...
    test "create_clock/1 with valid data creates a clock" do
      user = user_fixture()
      valid_attrs = %{status: true, time: ~N[2023-10-24 12:49:00]}

      assert {:ok, %Clock{} = clock} = Clocks.create_clock(user, valid_attrs)
      assert clock.status == true
      assert clock.time == ~N[2023-10-24 12:49:00]
      assert clock.user_id == user.id
    end

    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~N[2023-10-25 12:49:00]}

      assert {:ok, %Clock{} = clock} = Clocks.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~N[2023-10-25 12:49:00]
    end
